...
- агент оценивается не только как API, но и как memory-centric система принятия решений;
- в контрольной истории покрыты основные типы исходов: `conflict`, `no_conflict`, `uncertain`;
- в контрольной истории покрыты основные классы конфликтов: `character`, `fact`, `timeline`, `object`;
- отдельный акцент сделан на том, что новые сцены не переходят в подтверждённую память без `confirm`, а отклонённые сцены не должны участвовать в последующем анализе.
...
- наиболее чувствительные случаи: смешение `fact` и `event`, смешение фактов и описаний персонажа, нелинейные временные переходы и связанные состояния объектов.
